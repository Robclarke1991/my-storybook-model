...
class StableDiffusionXLInstantIDPipeline(
    ...
    def _encode_prompt(
        ...
        if do_classifier_free_guidance:
            ...
            if isinstance(negative_prompt, str):
                ...
                raise ValueError("`negative_prompt` and `prompt` must have the same batch size.")
            else:
        ...
        if do_classifier_free_guidance:
            ...
            if self.text_encoder is not None and (pooled_prompt_embeds is None or negative_pooled_prompt_embeds is None):
                raise ValueError("`pooled_prompt_embeds` and `negative_pooled_prompt_embeds` must be specified.")
    ...
    def check_inputs(self, prompt, image, height, width, callback_steps, negative_prompt=None, prompt_embeds=None, negative_prompt_embeds=None, ip_adapter_image_embeds=None):
        if height % 8 != 0 or width % 8 != 0:
            raise ValueError(f"`height` and `width` must be divisible by 8, but are {height} and {width}.")
        if callback_steps is not None and (not isinstance(callback_steps, int) or callback_steps <= 0):
            raise ValueError(f"`callback_steps` must be a positive integer, but is {callback_steps} of type {type(callback_steps)}.")
    ...
    def prepare_control_image(self, image, width, height, batch_size, num_images_per_prompt, device, dtype, do_classifier_free_guidance=False):
        ...
        image = image.repeat_interleave(batch_size * num_images_per_prompt, dim=0)
        image = image.to(device=device, dtype=dtype)
        if do_classifier_free_guidance:
    ...
    def __call__(
        ...
        if ip_adapter_image is not None:
            ...
            if ip_adapter_image_embeds is None:
                raise ValueError("Either `ip_adapter_image` or `ip_adapter_image_embeds` must be provided.")
            if do_classifier_free_guidance:
        ...
        latents = self.prepare_latents(
            batch_size * num_images_per_prompt, self.unet.config.in_channels, height, width,
        ...
        with self.progress_bar(total=num_inference_steps) as progress_bar:
            for i, t in enumerate(timesteps):
                ...
                if do_classifier_free_guidance:
                    added_cond_kwargs['text_embeds'] = torch.cat([negative_pooled_prompt_embeds, pooled_prompt_embeds], dim=0)
    ...
    def _get_add_time_ids(
        ...
        add_time_ids = torch.cat([add_time_ids, add_aesthetic_score, add_negative_aesthetic_score])
